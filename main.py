...
from models.modules import MSBE
from pipeline_brdm import StableDiffusionControlNetPipeline
...
def load_brdm_pipeline(args, accelerator, enable_xformers_memory_efficient_attention):
    ...
    unet = UNet2DConditionModel.from_pretrained(args.brdm_model_path, subfolder="unet")
    controlnet = ControlNetModel.from_pretrained(args.brdm_model_path, subfolder="controlnet")
    msbe = MSBE(3, 64, 4, 8, 32, 15, 4, 4, True).to(device='cuda')
    msbe.load_state_dict(torch.load(os.path.join(args.brdm_model_path, 'msbe', 'model.pth')))
    ...
    validation_pipeline = StableDiffusionControlNetPipeline(
        ...
        unet=unet, controlnet=controlnet, msbe=msbe, scheduler=scheduler, safety_checker=None, requires_safety_checker=False,
    )
    ...
    msbe.to(accelerator.device, dtype=weight_dtype)
...
def main(args, enable_xformers_memory_efficient_attention=True,):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("BRDM")

    pipeline = load_brdm_pipeline(args, accelerator, enable_xformers_memory_efficient_attention)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--brdm_model_path", type=str, default=None)
